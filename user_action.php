...
if(isset($_POST["action"]))
{
	...
	{
		$order_column = array('admin_name', 'admin_contact_no', 'admin_pseudo', 'admin_created_on');
		...
		if(isset($_POST["search"]["value"]))
		{
			...
			$search_query .= 'OR admin_pseudo LIKE "%'.$_POST["search"]["value"].'%" ';
	...
	if($_POST["action"] == 'Add')
	{
		...
		{
			$error = '<div class="alert alert-danger">User pseudo Already Exists</div>';
		}
		else
		{
			$user_image = '';
			...
			(admin_name, admin_contact_no, admin_pseudo, admin_password, admin_profile, admin_type, admin_created_on) 
			VALUES (:admin_name, :admin_contact_no, :admin_pseudo, :admin_password, :admin_profile, :admin_type, :admin_created_on)
			";
	...
	if($_POST["action"] == 'fetch_single')
	{
		...
		foreach($result as $row)
		{
			...
			$data['admin_pseudo'] = $row['admin_pseudo'];
	...
	if($_POST["action"] == 'Edit')
	{
		...
		else
		{
			...
			$data[':admin_pseudo'] = $_POST["admin_pseudo"];
